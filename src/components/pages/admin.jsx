...
const AdminClientes = () => {
  ...
  const guardarNuevoCliente = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        "https://hulk-gym-backend.vercel.app/api/auth/registrar-cliente",
        {
  ...
  const guardarClienteEditado = async () => {
    try {
      ...
      const res = await axios.put(
        `https://hulk-gym-backend.vercel.app/api/clientes/${clienteId}`,
  ...
  const confirmarEliminarCliente = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`https://hulk-gym-backend.vercel.app/api/clientes/${clienteAEliminar._id || clienteAEliminar.id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const renovarMembresia = async (cliente) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.put(`https://hulk-gym-backend.vercel.app/api/membresias/${cliente.id}/renovar`, {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const togglePagoMes = useCallback(async (cliente) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await axios.put(`https://hulk-gym-backend.vercel.app/api/membresias/${cliente._id}/pago`, {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const fetchEmailHistory = useCallback(async () => {
    try {
      ...
      const res = await axios.get("https://hulk-gym-backend.vercel.app/api/emails/", { headers: { Authorization: `Bearer ${token}` } });
      let history = [];
  ...
  const handleEliminarEmail = useCallback(async (emailId) => {
    ...
    if (result.isConfirmed) {
      try {
        const token = localStorage.getItem("token");
        await axios.delete(`https://hulk-gym-backend.vercel.app/api/emails/${emailId}`, { headers: { Authorization: `Bearer ${token}` } });
