...
const AdminClientes = () => {
  ...
  useEffect(() => {
    const verificarAuth = () => {
      ...
      console.log(
        "🔐 Admin: Verificando auth - userType:",
        userType,
        "token:",
        token ? "✅" : "❌"
      );
    ...
    const timer = setTimeout(verificarAuth, 100);
  ...
  const estadisticas = useMemo(() => {
    ...
    const activos = clientesConEmail.filter(
      (cliente) => getEstadoMembresia(cliente) === "Activo"
    ).length;
    const expiradas = clientesConEmail.filter(
      (cliente) => getEstadoMembresia(cliente) === "Expirada"
    ).length;
 ...
 const datosIngresos = useMemo(() => {
  ...
  clientesConEmail.forEach((cliente) => {
    try {
      // ✅ Usar fechaInicio o fechaCreacion como fallback
      let fechaStr = cliente.fechaInicio;
      
      if (!fechaStr && cliente.fechaCreacion) {
      ...
      // ✅ IMPORTANTE: Usar estadoCuenta en vez de estado
      if (cliente.estadoCuenta !== "Activo") return;
    ...
    () =>
      Math.max(
        ...(datosIngresos.length > 0
          ? datosIngresos.map((item) => item.valor)
          : [0])
      ) || 10000,
    [datosIngresos]
  );
  ...
  const calcularProgresoMembresia = useCallback((cliente) => {
    ...
    const diasRestantes = vencido
      ? 0
      : Math.ceil(Math.max(0, restanteMs) / (1000 * 60 * 60 * 24));
  ...
  const handleFormChange = useCallback((e) => {
    ...
    if (name === "fechaInicio" && value) {
      ...
      const inicioFormateado = `${fechaInicio
        .getDate()
        .toString()
        .padStart(2, "0")}/${(fechaInicio.getMonth() + 1)
        .toString()
        .padStart(2, "0")}/${fechaInicio.getFullYear()}`;
      const vencimientoFormateado = `${fechaVencimiento
        .getDate()
        .toString()
  ...
  if (!formData.dni?.trim()) {
    Swal.fire({ icon: "warning", title: "Campo requerido", text: "El DNI es obligatorio" });
    return;
  }
  if (!formData.email?.trim()) {
    Swal.fire({ icon: "warning", title: "Campo requerido", text: "El email es obligatorio para enviar las credenciales" });
    return;
  }
  ...
  try {
    ...
    const unMesDespues = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
      ...
      fechaInicio: formData.fechaInicio || hoy, // ⬅️ Fecha por defecto
      vencimiento: formData.vencimiento || unMesDespues, // ⬅️ Fecha por defecto
    ...
    // Actualizar lista local usando los datos del cliente devuelto
    const clienteCreado = res.data.cliente;
    const nuevoCliente = {
      id: clienteCreado.id,
      _id: clienteCreado.id,
      nombre: clienteCreado.nombre,
      dni: clienteCreado.dni,
      email: clienteCreado.email,
      fechaInicio: clienteCreado.fechaInicio,
      vencimiento: clienteCreado.vencimiento,
      estadoCuenta: clienteCreado.estadoCuenta || "Activo",
      precio: clienteCreado.precio || 10000,
      pagoMesActual: clienteCreado.pagoMesActual || false,
      cuentaActivada: clienteCreado.cuentaActivada || false
    };
    ...
    Swal.fire({
      ...
      html: `
        <div style="text-align: left;">
          <p><strong>${formData.nombre}</strong> ha sido registrado correctamente.</p>
          <hr>
          <p>📧 Se ha enviado un email a <strong>${formData.email}</strong> con:</p>
          <ul>
            <li>Credenciales de acceso temporales</li>
            <li>Enlace para cambiar contraseña</li>
          </ul>
          <p class="text-muted small">El cliente deberá cambiar su contraseña antes de poder acceder.</p>
        </div>
      `,
  ...
  const handleVerificarEmails = useCallback(async () => {
    ...
    try {
      ...
      const { verificarYNotificarExpiraciones } = await import(
            ...
            <div style="text-align: left; padding: 10px;">
              ...
              <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 10px;">
                ...
                <p style="margin: 8px 0;">
                  ...
                  <span style="color: #28a745;">${resultado.emailsEnviados}</span>
                </p>
                <p style="margin: 8px 0;">
                  <strong>Errores:</strong> 
                  <span style="color: ${
                    resultado.errores > 0 ? "#ffc107" : "#6c757d"
                  };">${resultado.errores}</span>
                </p>
              </div>
  ...
  const verificarVencimientosAutomaticos = useCallback(async () => {
    try {
      const { verificarVencimientosAutomaticos } = await import(
        "../../services/emailService"
      );
  ...
  // ✅ Coloca esto ANTES de verificarYResetearPagosMensuales (alrededor de línea 895)
...
const fetchEmailHistory = useCallback(async () => {
  try {
    ...
    console.log("📧 Respuesta historial emails:", res.data);
    let history = [];
...
const handleOpenEmailModal = useCallback(() => {
  ...
  async (cliente) => {
    const accion = cliente.pagoMesActual
      ? "marcar como NO pagado"
      : "marcar como pagado";
    ...
    Swal.fire({
      ...
      cancelButtonColor: "#6c757d",
      confirmButtonText: cliente.pagoMesActual
        ? "Sí, marcar NO pagado"
        : "Sí, confirmar pago",
      cancelButtonText: "Cancelar",
      reverseButtons: true,
      ...
      if (result.isConfirmed) {
        try {
          const token = localStorage.getItem("token");
          const nuevoEstadoPago = !cliente.pagoMesActual;
          ...
          await axios.put(
            ...
            {
              pagoMesActual: nuevoEstadoPago,
              fechaUltimoPago: nuevoEstadoPago ? new Date().toISOString() : cliente.fechaUltimoPago
            },
            {
              headers: { Authorization: `Bearer ${token}` }
            }
          ...
          const clienteActualizado = {
            ...cliente,
            pagoMesActual: nuevoEstadoPago,
            fechaUltimoPago: nuevoEstadoPago
          ...
          setClientes((prev) =>
            prev.map((c) => 
              (c._id === cliente._id || c.id === cliente.id) 
                ? clienteActualizado 
                : c
            )
          );

          // Actualizar también clienteSeleccionado si es el mismo
          if (clienteSeleccionado && 
              (clienteSeleccionado._id === cliente._id || clienteSeleccionado.id === cliente.id)) {
            setClienteSeleccionado(clienteActualizado);
          }

          Swal.fire({
            icon: "success",
            title: clienteActualizado.pagoMesActual
              ? "¡Pago confirmado!"
                ...
                {datosIngresos.map((item, index) => {
                  ...
                  const heightPx =
                    valorMaximo > 0
                      ...
                      : 0;
                  const hue =
                    200 - Math.floor((index / datosIngresos.length) * 70);
  ...
  const guardarClienteEditado = async () => {
    try {
      const token = localStorage.getItem("token");
      const clienteId = formData._id || formData.id;

      // ✅ URL completa + headers con token
      ...
      setClientes((prev) =>
        prev.map((c) => 
          (c._id === clienteId || c.id === clienteId) 
            ? res.data 
            : c
        )
      );
      ...
      if (clienteSeleccionado && 
          (clienteSeleccionado._id === clienteId || clienteSeleccionado.id === clienteId)) {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.mensaje || "No se pudo actualizar el cliente",
      });
    }
  };
  ...
  const confirmarEliminarCliente = async () => {
    try {
      const token = localStorage.getItem("token");
      // Usa el endpoint correcto y pasa el token en headers
      await axios.delete(
        `http://localhost:3000/api/clientes/${clienteAEliminar._id || clienteAEliminar.id}`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
      ...
      setClientes((prev) =>
        prev.filter(
          (c) =>
            c._id !== clienteAEliminar._id &&
            c.id !== clienteAEliminar.id
        )
      );
 ...
 const renovarMembresia = async (cliente) => {
  try {
    const token = localStorage.getItem("token");
    const res = await axios.put(
      `http://localhost:3000/api/membresias/${cliente.id}/renovar`,
      {},
      { headers: { Authorization: `Bearer ${token}` } }
    );

    setClientes((prev) =>
      prev.map((c) => (c.id === cliente.id ? { ...c, ...res.data.cliente } : c))
    );
    ...
    Swal.fire({
      icon: "error",
      title: "Error",
      text: error.response?.data?.mensaje || "No se pudo renovar la membresía",
    });
  }
};
            ...
            <Row className="g-3 mb-4">
              ...
              <Col xs={6} lg={3}>
                <StatCard
                  ...
                  icon={FaTimesCircle}
                  color={
                    estadisticas.membresiasVencidas > 0
                      ? "warning"
                      : "secondary"
                  }
                  isDarkMode={isDarkMode}
          ...
          Enviar notificaciones de vencimiento
              ...
              <Card.Body className="p-4">
                ...
                {!isMobile ? (
                  <div className="table-responsive">
                    ...
                    <Table
                      hover
                      className={`mb-4 ${isDarkMode ? "table-dark" : ""}`}
                    >
                      <thead
                        className={
                      ...
                      <tbody>
                        {clientesPaginados.clientes.length > 0 ? (
                          clientesPaginados.clientes.map((cliente) => (
                            <ClienteRow
                              key={cliente._id || cliente.id} // ✅ Agregar esto
                              cliente={cliente}
                    ...
                    {clientesPaginados.clientes.length > 0 ? (
                      ...
                      >
                        <FaUsers
                          size={48}
                          className={`mb-3 ${
                            isDarkMode ? "text-light opacity-50" : "text-muted"
                          }`}
                        />
                        <p
                          className={`mb-0 ${
                        ...
                        <Pagination.Next
                          onClick={() =>
                            setPaginaActual((prev) =>
                              Math.min(clientesPaginados.totalPaginas, prev + 1)
                            )
                          }
                          disabled={
            ...
            {clienteSeleccionado && (
              <Card
                ...
                id="detalle-cliente"
                className={`mt-5 border-0 shadow-lg overflow-hidden ${
                      ...
                      <div className="d-flex align-items-center mb-4">
                        ...
                        <div>
                          ...
                          <p className="mb-0 text-white text-opacity-75 fs-5">
                            Cliente #{clienteSeleccionado.id} • DNI:{" "}
                    ...
                    <Col xs={12} md={6}>
                      ...
                      >
                        <Card.Body>
                          ...
                          <Table
                            borderless
                            className={`mb-0 ${isDarkMode ? "table-dark" : ""}`}
                          >
                            <tbody>
                              <tr>
                                <td
                          ...
                          <div className="d-flex align-items-center mb-3">
                            ...
                            >
                              <FaDollarSign
                                size={18}
                                className={
                                  isDarkMode ? "text-success" : "text-success"
                                }
                              />
                            </div>
                            <h5 className="mb-0 fw-bold">
          ...
          <Form>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Text className="text-muted">
                <strong>Campo obligatorio</strong> - Necesario para
                notificaciones y gestión del cliente
              </Form.Text>
            </Form.Group>
            <Form.Group className="mb-3">
